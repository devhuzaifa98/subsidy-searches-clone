...
const Process = () => {
    ...
    return (
        <section id='process' className='container mt-10 md:mt-0 mx-auto min-h-screen flex flex-col justify-center items-center space-y-6'>
            ...
            <PrimaryButton text='qualify now!' onClick={() => scrollToTop()} />
        </section>
    )
}
    ...
    return (
        <div className="card max-w-sm mx-5 md:mx-0 bg-white border border-light rounded-lg shadow h-full flex flex-col relative">
            <div className="relative">
                <img className="card-img w-full rounded-t-lg object-cover object-top h-80" src={imageSrc} alt="" />
                <div className="absolute inset-0 bg-gradient-to-b from-secondary to-light opacity-50"></div>
            </div>
            <div className="p-5">
                <h5 className="mb-2 text-2xl font-bold tracking-tight text-secondary uppercase">{title}</h5>
            </div>
        </div>
    );
};
